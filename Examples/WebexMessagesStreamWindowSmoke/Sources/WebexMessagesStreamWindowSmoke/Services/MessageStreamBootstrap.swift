...
enum MessageStreamBootstrap {
    static func makeRuntime(configuration: StreamSmokeConfiguration) async throws -> MessageStreamRuntime {
        ...
        let stream = authorized.client.messages.stream(
        ...
        let connection = authorized.client.realtime.connect(options: WebexRealtimeOptions(
            ...
            deviceName: "webex-messages-stream-window-smoke"
        ))
    ...
    static func validateGrantedRealtimeScopes(
        ...
        guard missingScopes.isEmpty else {
            throw StreamSmokeError.missingRealtimeScopes(
