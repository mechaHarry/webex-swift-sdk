...
struct StreamSmokeConfiguration: Equatable {
    ...
    init(environment: [String: String]) throws {
        ...
        self.keychainService = environment["WEBEX_KEYCHAIN_SERVICE"] ?? "com.webex.swift-sdk.messages-stream-window-smoke"
    ...
    private static func required(
        ...
        guard let value = environment[name]?.trimmingCharacters(in: .whitespacesAndNewlines),
              !value.isEmpty else {
            throw StreamSmokeError.missingEnvironment(name)
        }
    ...
    private static func integer(
        ...
        guard let value = Int(rawValue),
              value >= minimum,
              value <= maximum else {
            throw StreamSmokeError.invalidInteger(name: name, value: rawValue, minimum: minimum, maximum: maximum)
        }
...
enum StreamSmokeError: Error, Equatable, CustomStringConvertible {
